...
namespace forTeacher.DataAcess
{
    public class DAO
    {
        ...
        public static bool updateLab1Mark(int id, string lab1)
        {
            string sql;
            if (lab1.Length > 0)
                sql = "UPDATE Marks   SET  [Lab1] = @l1  WHERE ID = @id";
            else
                sql = "UPDATE Marks   SET  [Lab1] = NULL  WHERE ID = @id";
        ...
        public static bool updateLab2Mark(int id, string lab2)
        {
            string sql;
            if (lab2.Length > 0)
                sql = "UPDATE Marks   SET  [Lab2] = @l2  WHERE ID = @id";
            else
                sql = "UPDATE Marks   SET  [Lab2] = NULL  WHERE ID = @id";
        ...
        public static bool updateLab3Mark(int id, string lab3)
        {
            string sql;
            if (lab3.Length > 0)
                sql = "UPDATE Marks   SET  [Lab3] = @l3  WHERE ID = @id";
            else
                sql = "UPDATE Marks   SET  [Lab3] = NULL  WHERE ID = @id";
        ...
        public static bool updateLab4Mark(int id, string lab4)
        {
            string sql;
            if (lab4.Length > 0)
                sql = "UPDATE Marks   SET  [Lab4] = @l4  WHERE ID = @id";
            else
                sql = "UPDATE Marks   SET  [Lab4] = NULL  WHERE ID = @id";
        ...
        public static bool updatePT2Mark(int id, string pt2)
        {
            string sql;
            if (pt2.Length > 0)
                sql = "UPDATE Marks   SET  ProgressTest2 = @pt2  WHERE ID = @id";
            else
                sql = "UPDATE Marks   SET  ProgressTest2 = NULL WHERE ID = @id";
        ...
        public static bool updatePT1Mark(int id, string pt1)
        {
            string sql;
            if (pt1.Length > 0)
                sql = "UPDATE Marks   SET  ProgressTest1 = @pt1  WHERE ID = @id";
            else
                sql = "UPDATE Marks   SET  ProgressTest1 = NULL WHERE ID = @id";
        ...
        //-----------------------------ADMIN-----------------------------------------------------------------//
        public static DataTable getSem()
        {
            string sql = "select * from Semesters order by Semesters.[From]";
            return GetDataBySQL(sql);

        }
        public static DataTable getSubject1(string semesterID)
        {
            string sql = "select Subjects.SubjectID,Subjects.SubjectName from AClass join Subjects on AClass.SubjectID = Subjects.SubjectID where SemesterID = '" + semesterID + "' group by Subjects.SubjectID,Subjects.SubjectName";
            return GetDataBySQL(sql);
        }
